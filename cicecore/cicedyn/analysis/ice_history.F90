...
! 2010 Alison McLaren and ECH: Added 3D capability
      ...
      use ice_constants, only: c0, c1, c2, c100, c360, c180, &
          p001, p25, p5, mps_to_cmpdy, kg_to_g, spval_dbl
      ...
      ! tides
      ! Hard-wired daily tidal-current diagnostic history indices
      integer (kind=int_kind), dimension(max_nstrm) :: &
           n_tide_u_eff          = 0, &
           n_tide_v_eff          = 0, &
           n_tide_speed_eff      = 0, &
           n_tide_speed_daymax   = 0, &
           n_tide_speed_dayrms   = 0, &
           n_tide_n_subday       = 0, &
           n_tide_n_curr_over_fi = 0, &
           n_tide_n_capped       = 0
...
!         William H. Lipscomb, LANL
      ...
      real (kind=dbl_kind), intent(in) :: &
         dt      ! time step
      ! tides
      character(len=max_nstrm) :: f_tide_diag
...
!!!!! end example
         ...
         ! tides
      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_u_eff,"tide_u_eff","m/s",tstr2D, tcstr, &
            "applied tidal u-current perturbation", &
            "after depth limiter, ramp and cap", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_v_eff,"tide_v_eff","m/s",tstr2D, tcstr, &
            "applied tidal v-current perturbation", &
            "after depth limiter, ramp and cap", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_speed_eff,"tide_spd","m/s",tstr2D, tcstr, &
            "applied tidal-current speed", &
            "after depth limiter, ramp and cap", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_speed_daymax,"tide_spd_max","m/s",tstr2D, tcstr, &
            "daily maximum tidal-current speed", &
            "sub-daily max of applied tidal-current perturbation", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_speed_dayrms,"tide_spd_rms","m/s",tstr2D, tcstr, &
            "daily RMS tidal-current speed", &
            "sub-daily RMS of applied tidal-current perturbation", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_n_subday,"tide_n_sub","1",tstr2D, tcstr, &
            "number of tide diagnostic samples", &
            "sub-daily sample count accumulated during current day", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_n_curr_over_fi,"tide_n_over","1",tstr2D, tcstr, &
            "number of samples exceeding FI speed threshold", &
            "count where applied tide speed exceeds tide_curr_diag_thresh", c1, c0, ns1, f_tide_diag)

      if (f_tide_diag(1:1) /= 'x') &
         call define_hist_field(n_tide_n_capped,"tide_n_cap","1",tstr2D, tcstr, &
            "number of samples limited by tide speed cap", &
            "count where tide_speed_cap modified applied current", c1, c0, ns1, f_tide_diag)
...
! author:   Elizabeth C. Hunke, LANL
      ...
      use ice_forcing, only: &
           tide_u_eff, tide_v_eff, tide_speed_eff, &
           tide_speed_daymax, tide_speed_dayrms, &
           tide_n_subday, tide_n_curr_over_fi, tide_n_capped
      ...
      do iblk = 1, nblocks
         ...
         if (allocated(tide_speed_eff)) then
            call accum_hist_field(n_tide_u_eff,     iblk, tide_u_eff(:,:,iblk),     a2D)
            call accum_hist_field(n_tide_v_eff,     iblk, tide_v_eff(:,:,iblk),     a2D)
            call accum_hist_field(n_tide_speed_eff, iblk, tide_speed_eff(:,:,iblk), a2D)
         endif
      ...
      !---------------------------------------------------------------
           ...
           do i = ilo, ihi
              if (.not. tmask(i,j,iblk)) then ! mask out land points
                 ...
                 ! tides
                 if (n_tide_speed_daymax  (ns) /= 0) a2D(i,j,n_tide_speed_daymax(ns),  iblk) = spval_dbl
                 if (n_tide_speed_dayrms  (ns) /= 0) a2D(i,j,n_tide_speed_dayrms(ns),  iblk) = spval_dbl
                 if (n_tide_n_subday      (ns) /= 0) a2D(i,j,n_tide_n_subday(ns),      iblk) = spval_dbl
                 if (n_tide_n_curr_over_fi(ns) /= 0) a2D(i,j,n_tide_n_curr_over_fi(ns),iblk) = spval_dbl
                 if (n_tide_n_capped      (ns) /= 0) a2D(i,j,n_tide_n_capped(ns),      iblk) = spval_dbl
              else
                 ...
                 if (n_tide_n_capped      (ns) /= 0) a2D(i,j,n_tide_n_capped(ns),      iblk) = spval_dbl
                 if (allocated(tide_speed_eff)) then
                    if (n_tide_speed_daymax(ns) /= 0) &
                       a2D(i,j,n_tide_speed_daymax(ns),iblk) = tide_speed_daymax(i,j,iblk)
                    if (n_tide_speed_dayrms(ns) /= 0) &
                       a2D(i,j,n_tide_speed_dayrms(ns),iblk) = tide_speed_dayrms(i,j,iblk)
                    if (n_tide_n_subday(ns) /= 0) &
                       a2D(i,j,n_tide_n_subday(ns),iblk) = tide_n_subday(i,j,iblk)
                    if (n_tide_n_curr_over_fi(ns) /= 0) &
                       a2D(i,j,n_tide_n_curr_over_fi(ns),iblk) = tide_n_curr_over_fi(i,j,iblk)
                    if (n_tide_n_capped(ns) /= 0) &
                       a2D(i,j,n_tide_n_capped(ns),iblk) = tide_n_capped(i,j,iblk)
                 else
                    if (n_tide_speed_daymax(ns) /= 0) &
                       a2D(i,j,n_tide_speed_daymax(ns),iblk) = spval_dbl
                    if (n_tide_speed_dayrms(ns) /= 0) &
                       a2D(i,j,n_tide_speed_dayrms(ns),iblk) = spval_dbl
                    if (n_tide_n_subday(ns) /= 0) &
                       a2D(i,j,n_tide_n_subday(ns),iblk) = spval_dbl
                    if (n_tide_n_curr_over_fi(ns) /= 0) &
                       a2D(i,j,n_tide_n_curr_over_fi(ns),iblk) = spval_dbl
                    if (n_tide_n_capped(ns) /= 0) &
                       a2D(i,j,n_tide_n_capped(ns),iblk) = spval_dbl
                 endif
              endif
           enddo                ! i
           enddo                ! j
